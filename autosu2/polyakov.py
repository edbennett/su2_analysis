...
PLAQ_TO_IDX = {(1, 0): 0, (2, 0): 1, (3, 0): 2, (2, 1): 3, (3, 1): 4, (3, 2): 5}
...
def get_loops_from_raw(filename, first_config):
    ...
    for line in open(filename):
        ...
        if line_contents[0] == "[IO][0]Configuration" and line_contents[2] == "read":
            ...
            configs.append(int(findall(r".*n(\d+)(?:_[0-9.]+)?]", line_contents[1])[0]))
            continue
        ...
        if line_contents[0] == "[PLAQ][0]Plaq(":
            ...
            directions = findall(r"\[PLAQ\]\[0\]Plaq\((\d),(\d)\)", line_contents[0])[0]
            dir1 = int(directions[0])
            dir2 = int(directions[1])
            plaqs[PLAQ_TO_IDX[(dir1, dir2)]].append(
                float(line_contents[3]) + float(line_contents[5]) * 1j
            )
        elif line_contents[0] in (
            "[POLYAKOV][0]Polyakov",
            "[FUND_POLYAKOV][0]Polyakov",
        ):
            dir1 = int(line_contents[2])
            loops[dir1].append(float(line_contents[4]) + float(line_contents[5]) * 1j)
...
def curve_fit_with_minimize(
    f, xdata, ydata, p0, sigma=None, absolute_sigma=False, bounds=None
):
...
def fit_and_plot_polyakov_loops(
    filename,
    bin_width=None,
    num_bins=None,
    first_config=0,
    ...
    histogrammed_loops = [histogram(loop.real, bins=bins)[0] for loop in loops]
    ...
    if do_fit:
        ...
        fitted_params = [
            tuple(param[0] for param in param_set)
    ...
    plot_loops(
        (bin_lower_edges, histogrammed_loops),
...
def fit_plot_and_save_polyakov_loops(
    ...
    for direction, fit_result_set in enumerate(fit_results):
        for observable, result in zip(("amplitude", "centre", "sigma"), fit_result_set):
            add_measurement(
                simulation_descriptor, f"polyakov_{direction}_{observable}", *result
            )
    return fit_results
