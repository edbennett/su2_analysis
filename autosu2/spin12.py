...
def arrayify_corr(result):
    metadata = set([
        key
        for cfg_result in result.values()
        for key in cfg_result['metadata']
    ])
    new_result = {
        'metadata': {}
    }
    for attr in metadata:
        new_result['metadata'][attr] = asarray([
            cfg_result['metadata'][attr]
            for _, cfg_result in sorted(result.items())
        ])

    channels = set([
        key
        for cfg_result in result.values()
        for key in cfg_result
    ]) - {'metadata'}
...
def arrayify(results):
    ...
    for observable, observable_results in results.items():
        if (
                observable.endswith('plaquette')
                or observable.startswith('polyakov_line')
        ):
...
def get_correlators_spin12format(filename):
    ...
    for i, line in enumerate(open(filename, 'r')):
        ...
        if observable.endswith('plaquette'):
            ...
            results[observable][cfg_index] = tuple_to_complex(
                split_line[3]
            )
        elif observable == 'Meson_corr':
            process_meson_corr_line(
                split_line,
                results[observable][cfg_index]
            )
        elif observable == 'gluinoglue':
            process_gluinoglue_line(
                split_line,
                results[observable][cfg_index]
            )
