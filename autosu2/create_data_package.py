...
from .do_analysis import (
    filter_complete, get_file_contents, get_subdirectory_name
)
...
def modenumber_getter(filename_base, group, **params):
    ...
    modenumbers_array = asarray([
        [
            modenumbers[upper_bound][trajectory]
    ...
    for valence_mass in set(correlators.valence_mass):
        ...
        for channel in set(correlators.channel):
            ...
            channel_subgroup.create_dataset(
                'config_indices',
                data=subset_correlators.trajectory.to_numpy()
            )
            channel_subgroup.create_dataset(
                'correlators',
...
def main():
    ...
    ensembles = filter_complete(
        yaml.safe_load(get_file_contents(args.ensembles_file))
    )
