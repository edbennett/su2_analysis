...
def do_plot(data, ensemble=None, filename=None, ax=None):
    ...
    gammas = gammas.dropna(axis='index',
                       subset=('gamma', 'err(stat)'))
    ...
    cbax = inset_axes(
        ax, width='50%', height='10%', loc='lower right', borderpad=1
    )
    cb = plt.colorbar(
    ...
    ax.scatter(gammas.xmin.values, gammas.gamma.values,
