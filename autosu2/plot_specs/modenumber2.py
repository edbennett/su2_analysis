...
def do_plot(
    ...
    data = data.dropna(axis="index", subset=("gamma_star", "gamma_star_error"))
    ...
    cbax = inset_axes(ax, width="70%", height="10%", loc="lower right", borderpad=1)
    cb = plt.colorbar(
...
def fit_modenumber_plateau(data, ensemble):
    ...
    fit_result, fit_variance = curve_fit(
        ...
        sigma=(
            data_to_fit.gamma_star_error.values * badness_to_weight(data_to_fit.badness)
        ),
        absolute_sigma=True,
    )
...
def generate(data, ensembles):
    ...
    for ensemble_name, ax in zip(ensembles_to_plot, axes):
        ...
        modenumber_data["window_length"] = (
            modenumber_data.omega_upper_bound - modenumber_data.omega_lower_bound
        )
