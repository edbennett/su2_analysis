...
def fit(merged_data, betas_to_fit, gamma_star=None):
    ...
    def fit_form(all_mpcac, p):
        ...
        for beta, (L, mass) in all_mpcac.items():
            x = L * mass ** (1 / ym)
            LMH[beta] = quadratic(x, p["alpha0"], p["alpha1"], p["alpha2"]) * (
                1 + p["c"][betas_to_fit.index(beta)] * mass ** (-p["y0"] / ym)
            )
        return LMH
    ...
    mg5_dict = {
        beta: (
            L[beta_values == beta]
            * gv.gvar(
                mg5[beta_values == beta], np.diag(mg5_e[beta_values == beta]) ** 2
            )
        )
        for beta in betas_to_fit
    }
    ...
    for betas_to_fit in betas_to_fit_set:
        ...
        for gamma_star in np.linspace(0.1, 1.05, 20):
            ...
            plot(
                ...
                betas_to_fit,
                filename.format(
                    "auxiliary", len(betas_to_fit), f"_gamma{gamma_star:.2f}"
                ),
            )
