...
def get_basic_metadata(ensembles_filename):
    ...
    metadata["_comment"] = "This file and all the files in this directory were generated automatically. Do not modify them; re-run the analysis workflow!"
    metadata["workflow_run"] = {
        "completed": datetime.now(timezone.utc).isoformat(),
        "user_name": os.getlogin(),
        "machine_name": socket.gethostname()
    }
    metadata["ensembles_metadata"] = {"last_updated": datetime.fromtimestamp(ensembles_file.stat().st_mtime, timezone.utc).isoformat()}
